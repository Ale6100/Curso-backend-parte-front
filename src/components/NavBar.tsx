...
const NavBar = () => { // Perdón por la navbar gigante. Tengo pendiente achicar código acá
    const personalContext = useContext(PersonalContext)
    if (!personalContext) return null

    const { user, productsInCart } = personalContext
    ...
    useEffect(() => { 
        ...
        const fondoDifuminado = document.getElementById(`fondoDifumi nadoResponsive`)
        const botonNavBar = document.getElementById("botonNavBar") as HTMLElement
    ...
    const MiniComponenteMenuPerfil = ({ text }: { text: string }) => {
        if (!user) {
            if (text === "navPC") {
                return (
