...
const OneProduct = ({ product }: { product: ProductInterface }) => {
    return (
        <div className={"p-1 flex flex-col justify-evenly text-center w-40 h-96 border-2 border-black rounded-sm"}>
            <div className='h-48 flex justify-center items-center'>
                <img src={product.image.includes("http") ? product.image : `${import.meta.env.VITE_BACKEND_URL}/`+product.image} alt="Image product" className='max-h-48 border border-blue-600 rounded-sm'/>
            </div>
