...
const CrudAdmins = () => {
    ...
    const [ tituloElegido, setTituloElegido ] = useState<string>("")
    const [ infoProduct, setInfoProduct ] = useState<ProductInterface | null>(null)
    ...
    const FirstForm = () => {
        const submitFirstForm = async (e: React.FormEvent<HTMLFormElement>) => {
            e.preventDefault()

            const formData = new FormData(e.currentTarget)
            const titleInput = formData.get("title") as string
            const seleccion = formData.get("action") as string
    ...
    return (
        <div className='m-5'>
            ...
            <div> {/* Dependiendo del valor elegido en el FirstForm, se renderiza alguno de los otros cuatro componentes, correspondientes al CRUD de productos*/}
                ...
                selectedValue === "read" ? <ReadProduct infoProduct={infoProduct} /> : 
                selectedValue === "update" ? <UpdateProduct infoProduct={infoProduct}/> :
                selectedValue === "delete" && <DeleteProduct infoProduct={infoProduct} />}
            </div>
        </div>
    );
}
